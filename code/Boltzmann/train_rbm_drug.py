...
drug_dataset = CustomDataset("../data/drug2fingerprint.txt")
from torch import nn
import torch
# DataLoader is used to load the dataset
# for training
drug_loader = torch.utils.data.DataLoader(TensorDataset(torch.Tensor(drug_dataset).to(DEVICE)),
                                     batch_size=16,
                                     shuffle=True)
...
new_train_dl = drug_loader

visible_dim = DRUG_DIMENSION
hidden_dim = None
...
for epoch in range(num_epochs):
    losses = []
    for i, data_list in enumerate(drug_loader):
        data = data_list[0]
        v_pred = dae(data)
...
torch.save(dae, "../model/rbm_drug_1"+ str(num_epochs))


# plt.style.use('fivethirtyeight')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Error plot for {}'.format("DRUG_RBM_AE"))
plt.legend("Train MSE error")
plt.plot(epoch_loss[-1 * num_epochs:])
plt.savefig("drug_rbm_loss" + ".png")
plt.show()

plt.plot(epoch_loss)
plt.savefig("drug_rbm_loss_1" + ".png")
plt.show()
